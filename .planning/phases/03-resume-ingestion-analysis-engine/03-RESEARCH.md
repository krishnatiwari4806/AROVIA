...
- **RESM-04:** Gemini structured schema extraction parses raw text into validated Pydantic models (`skills`, `experience_years`, `domains`, `education`, `summary`) using `google-genai` SDK only with native JSON schema enforcement, 1 automatic retry, and clean HTTP 503 error handling on provider outages.
...
4. **`google-genai` (`>=0.1.0`)**: Official Google GenAI SDK (`from google import genai`) exclusively used for invoking Gemini models with structured JSON schemas (`response_schema`).

---

## 3. Architecture & Security Invariants

### 3.1 Defensive File Ingestion & Atomic Replacement
...
- **Atomic Replacement Lifecycle**:
  - Do NOT delete the existing resume file or database record before the new resume has fully succeeded across all validation, extraction, Gemini parsing, disk persistence, and database commit stages.
  ...
  - **Rollback Guard**: If database commit fails, delete the newly created file from disk, rollback the transaction, and keep the existing resume intact.
- **Isolated Storage**: Save validated files to `storage/resumes/<uuid>.<ext>` with non-guessable UUIDs and strict `0600` permissions.
...
### 3.3 Gemini Structured JSON Schema (`google-genai` Only)
...
3. **Gemini Structured Parsing Tests (`test_resume_parser.py`)**:
   - Mocked `google-genai` SDK client returning structured JSON schema.
...
4. **Resume CRUD & Atomic Replacement Tests (`test_resume_endpoints.py`)**:
   - `POST /api/v1/resumes/upload`: Atomically replaces prior resume file on disk and DB record only after all stages succeed.
   - Atomic failure test: If parsing/extraction fails during re-upload, existing resume remains untouched and accessible.
